...
public class PushAbilityAliyunImpl implements PushAbility {
    ...
    @Override
    public boolean sendMessage(String account, String title, String body, JSONObject json) {
        return send("MESSAGE", account, title, body, json);
    }

    @Override
    public boolean sendNotice(String account, String title, String body, JSONObject json) {
        return send("NOTICE", account, title, body, json);
    }

    private boolean send(String type, String account, String title, String body, JSONObject json) {
        ...
        pushRequest.setPushType(type); // 消息类型 MESSAGE NOTICE
...
//        pushRequest.setAndroidPopupBody("Popup Body");
        if (json != null) {
            pushRequest.setAndroidExtParameters(json.toJSONString()); //设定通知的扩展属性。(注意 : 该参数要以 json map 的格式传入,否则会解析出错)
        }
//        推送控制
...
//        pushRequest.setStoreOffline(true); // 离线消息是否保存,若保存, 在推送时候，用户即使不在线，下一次上线则会收到
        try {
            ...
            logger.info("RequestId:{}, MessageID:{}\n", pushResponse.getRequestId(), pushResponse.getMessageId());
            return true;
        } catch (ClientException e) {
            logger.error("aliyun push error", e);
            return false;
        }
    }
}
